import { $, $$, uid, todayISO, clamp, daysBetween, fmtUSD, asNumber, hexToRgb, h, showToast, clampDay, nextMonthlyDateFrom } from "./utils.js";
import { sectionTitle, kpi, fieldInput, currencyInput, percentInput, showModal } from "./ui-helpers.js";
...
function appSidebar(){
  ...
  return h('aside',{class:'sidebar'},
    ...
    h('nav',{class:'nav'},
      ...
      item('credit','💳','Credit Cards'),
      item('financials','🏦','Financials'),
      item('settings','⚙️','Settings')
    ),
    h('div',{class:'spacer'}),
    h('div',{class:'foot'},
      collapsed? h('div',{class:'muted',style:'text-align:center;'}, state.theme==='dark'?'🌙':'☀️') :
      ...
      const cardBox=h('div',{class:'card',style:baseStyle},
        ...
        h('div',{style:'display:flex;justify-content:space-between;align-items:center;gap:8px;'},
          h('div',null, h('div',{style:'font-weight:700;font-size:16px;'}, card.name),
            h('div',{class:'muted'}, (card.issuer||'Issuer')+' • Limit: '+(card.limit?fmtUSD(card.limit):'(set limit)'))),
        ...
        h('div',{class:'grid grid-2',style:'margin-top:8px;'},
          ...
          fieldBlock('Over Limit', last?.overLimit? fmtUSD(last.overLimit) : '—', (last?.overLimit||0)>0),
          fieldBlock('Utilization', util!=null ? (util*100).toFixed(1)+'%' : '—', util!=null && util>=0.9),
          fieldBlock('Util Target', card.utilTarget!=null ? (card.utilTarget+'%') : '—'),
          fieldBlock('Due Date', last?.dueDate || '—'),
          fieldBlock('Days till Due', daysTilDue != null? String(daysTilDue) : '—', daysTilDue != null && daysTilDue<=5),
          fieldBlock('Statement Close', last?.statementEnd||'—'),
          fieldBlock('Closes in', closesIn != null? String(closesIn) : '—', closesIn != null && closesIn<=3),
          fieldBlock('Est. Interest (mo)', estInterest!=null? fmtUSD(estInterest) : '—')
        )
      );
      return cardBox;
    }))
...
function finAccounts(){
  ...
  state.finAccounts.forEach(a=>{
    grid.appendChild(h('div',{class:'panel p4'},
      sectionTitle(a.name+' — '+(a.type==='bank'?'Bank':'Loan')),
      h('div',{class:'grid grid-3'},
...
function finLog(){
  ...
  const right=h('div',{class:'panel p4',style:'margin-top:10px;'}, sectionTitle('History'),
    pagedHistory(state.finEntries, 200, (e)=>{
      ...
      return h('tr',null,
        h('td',null,e.date), h('td',null,a?.name||'—'), h('td',null,fmtUSD(e.balance)), h('td',null,fmtUSD(e.inflow)), h('td',null,fmtUSD(e.outflow)),
        h('td',null,h('div',{title:e.notes,style:'max-width:360px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;'},e.notes||'')),
...
function FinUpcomingWidget(){
  ...
  upcoming.forEach(x=>{ const acc=getAcc(x.accountId);
    list.appendChild(h('div',{style:'display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;background:var(--panel-2);border:1px solid var(--border);padding:8px 12px;border-radius:10px;'},
      h('div',{class:'muted'}, `${x.date} • ${x.desc||'Expense'} • ${acc?.name||'—'}`),
...
function finUpcoming(){
  ...
  const tbl=h('table',{class:'table'}, h('thead',null,h('tr',null, ...['Date','Account','Amount','Description','Paid',''].map(th=>h('th',null,th)))),
    h('tbody',null, ...upcoming.map(x=>{
      ...
      return h('tr',null, h('td',null,x.date), h('td',null,acc?.name||'—'), h('td',null,fmtUSD(x.amount)), h('td',null,x.desc||''), h('td',null,x.paid?'Yes':'No'),
